...
make_seer_summary_latex <- function(hcc_alldata) {
  ...
  c(
    ...
    "\\vspace{0.5em}",
    "\\footnotesize Continuous variables are presented as median [IQR]. Categorical variables are presented as n (\\%).",
    "\\end{minipage}",
    "\\end{table}"
  )
}
...
make_seer_risk_group_latex <- function(summary_table) {
  ...
  c(
    ...
    "\\begin{minipage}{0.95\\linewidth}",
    "\\vspace{0.5em}",
    "\\footnotesize Continuous variables are presented as median [IQR]. Categorical variables are presented as n (\\%). P-values are from Wilcoxon rank-sum tests for continuous variables and chi-square or Fisher's exact tests for categorical variables.",
    "\\end{minipage}",
    "\\end{table}"
  )
}
