...
summary_s2 = readRDS("results/SimulationS1_result_IBS.rds")
summary_s3 = readRDS("results/SimulationS2_result_IBS.rds")
...
make_row_plot <- function(data, sample_size, sim_label, show_legend = FALSE) {
  ...
  p <- ggplot(plot_data, aes(x = Method, y = mean_IBS, fill = Plot_Method)) +
    geom_bar(stat = "identity", width = 0.7) +
    geom_errorbar(
      aes(ymin = mean_IBS - sd_IBS, ymax = mean_IBS + sd_IBS),
      width = 0.25, linewidth = 0.4
    ) +
    facet_wrap(~ facet_label, nrow = 1) +
    scale_fill_manual(values = method_colors, breaks = method_levels, labels = method_labels) +
    coord_cartesian(ylim = c(0.0, 0.65)) +
    labs(x = NULL, y = "IBS", fill = "Method") +
...
ggsave(
  filename = "results/figures/Simulation_IBS.pdf",
  plot = plot_IBS,
  width = 9,
  height = 10.5,
  units = "in",
