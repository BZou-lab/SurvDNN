...
# Working Directory
Fold_Name = "/work/users/{o}/{n}/{onyen}/Permfit_Sim/formal_newpermfits3_lowcor_3000_sqrt3/"
...
if (DataN_Scov %in% 
  ...
  sims_beta = c(1,2,0.5,1,rep(0,p-1))/sqrt(3)
...
##### 2.  Simulate Covariate without covarience ######
...
if (DataN_Ncov %in% 
  ...
  X_Ncov = data.frame(mvrnorm(n=N,mu=rep(0,p_block),
                              Sigma = (1-rho)*diag(p_block)+rho),
                      mvrnorm(n=N,mu=rep(0,p_block),
                              Sigma = (1-rho)*diag(p_block)+rho),
                      mvrnorm(n=N,mu=rep(0,p_block),
                      ...
                      mvrnorm(n=N,mu=rep(0,p_block),
                              Sigma = (1-rho)*diag(p_block)+rho))
  colnames(X_Ncov) = paste("X",1:(4*p_block),sep = "")
  
  for (z in 1:p_block) {
    if (z==1){
      Z_Ncov = data.frame(rbinom(N,1,0.4))
    }else{
      Z_random = data.frame(rbinom(N,1,0.4))
      Z_Ncov = cbind(Z_Ncov,Z_random)
    }
  }
  
  colnames(Z_Ncov) = paste("Z",1:(p_block),sep = "")
  
  C_Ncov = cbind(X_Ncov,Z_Ncov)
  ...
  C_Ncov[,"X1Z2"] = C_Ncov[,"X1"]*C_Ncov[,"Z2"]
  C_Ncov[,paste("X",p_block+1,"Square",sep = "")] = (C_Ncov[,paste("X",p_block+1,sep = "")])^2
  C_Ncov[,paste("X",2*p_block+1,"X",3*p_block+1,sep = "")] = C_Ncov[,paste("X",2*p_block+1,sep = "")]*C_Ncov[,paste("X",3*p_block+1,sep = "")]
  
  # Generate Survival Time
  
  Y_Ncov= simsurv(dist = "gompertz",lambdas = lambda.t,gammas = alpha.t,
                  betas = sims_beta,
                  x= C_Ncov)
  
  C_Ncov$id=1:nrow(C_Ncov)
  Full_Ncov = left_join(Y_Ncov,C_Ncov,by = "id")
  ...
  Full_Ncov$censor_time = rweibull(N,shape = alpha.c,scale = theta)
  
  Full_Ncov$eventtime1 = apply(Full_Ncov,1,function(x){
    Cens = x["censor_time"]
    Event = x["eventtime"]
    return(min(c(Cens,Event)))
  })
  
  Full_Ncov$status1 = apply(Full_Ncov,1,function(x){
    Cens = x["censor_time"]
    Event = x["eventtime"]
    if (Event >= Cens){
      return(0)
  ...
  saveRDS(Full_Ncov,file = DataN_Ncov)
}
...
if (FileN_Ncov %in% 
    list.files(paste(Fold_Name,Folder_args,sep = ""),full.names = T) == F){
  Perm_dnnet1_Ncov = permfit_survival(train = Train_Ncov,n_perm =100,method = "dnnet",
                                      k_fold = 5,
                                      n.hidden = c(50, 40, 30, 20),family = "coxph",
                                      l1.reg = 10**-4, early.stop.det = 1000, n.batch = 50,
                                      n.epoch = 1000, learning.rate.adaptive = "adam", 
  ...
  saveRDS(Perm_dnnet1_Ncov,file = FileN_Ncov)
  rm(Perm_dnnet1_Ncov)
}
...
if (FileN_Ncov %in% 
    list.files(paste(Fold_Name,Folder_args,sep = ""),full.names = T) == F){
  Perm_dnnet_Ncov = permfit_survival(train = Train_Ncov,n_perm =100,method = "ensemble_dnnet",
                                     k_fold = 5,
                                     n.ensemble = 100, esCtrl = esCtrl) %>% try()
  saveRDS(Perm_dnnet_Ncov,file = FileN_Ncov)
  rm(Perm_dnnet_Ncov)
}
...
if (FileN_Ncov %in% 
    list.files(paste(Fold_Name,Folder_args,sep = ""),full.names = T) == F){
  Perm_xgb_Ncov = permfit_survival(train = Train_Ncov,n_perm =100,method = "Xgboost"
                                   ,k_fold = 5,nrounds = 50) %>% try()
  FileN_Ncov = paste(Fold_Name,Folder_args,"/PermFit_xgb_Ncov","_",
                     sim_num,"_",use_seed,"_",p_block,".rds",sep = "")
  saveRDS(Perm_xgb_Ncov,file = FileN_Ncov)
  print(Sys.time())
}
...
if (FileN_Ncov %in% 
    list.files(paste(Fold_Name,Folder_args,sep = ""),full.names = T) == F){
  Perm_rsf_Ncov = permfit_survival(train = Train_Ncov,n_perm =100,method = "random_forest",
                                   k_fold = 5,ntrees = 500) %>% try()
  saveRDS(Perm_rsf_Ncov,file = FileN_Ncov)
  rm(Perm_rsf_Ncov)
}
...
FileN_Scov = paste(Fold_Name,Folder_args,"/PermFit_svm_Scov","_",
                   sim_num,"_",use_seed,"_",p_block,".rds",sep = "")
FileN_Ncov = paste(Fold_Name,Folder_args,"/PermFit_svm_Ncov","_",
                   sim_num,"_",use_seed,"_",p_block,".rds",sep = "")

if (FileN_Scov %in% 
    list.files(paste(Fold_Name,Folder_args,sep = ""),full.names = T) == F){
  Perm_svm_Scov = permfit_survival(train = Train_Scov,n_perm =100,method = "Survival_SVM",
                                   k_fold = 5,opt.meth = "quadprog",kernel = "add_kernel",
...
if (FileN_Ncov %in% 
    list.files(paste(Fold_Name,Folder_args,sep = ""),full.names = T) == F){
  Perm_svm_Ncov = permfit_survival(train = Train_Ncov,n_perm =100,method = "Survival_SVM",
                                   k_fold = 5,opt.meth = "quadprog",kernel = "add_kernel",
                                   gamma.mu = 0.1,type = "vanbelle2",diff.meth = "makediff3") %>% try()
  saveRDS(Perm_svm_Ncov,file = FileN_Ncov)
  rm(Perm_svm_Ncov)
}
...
if (FileN_Ncov %in% 
    list.files(paste(Fold_Name,Folder_args,sep = ""),full.names = T) == F){
  Perm_cox_Ncov = permfit_survival(train = Train_Ncov,n_perm =100,method = "survival_cox",
                                   k_fold = 5) %>% try()
  saveRDS(Perm_cox_Ncov,file = FileN_Ncov)
  rm(Perm_cox_Ncov)
}
...
if (FileN_Ncov %in% 
    list.files(paste(Fold_Name,Folder_args,sep = ""),full.names = T) == F){
  Perm_aft_Ncov = permfit_survival(train = Train_Ncov,n_perm =100,method = "survival_aft",
                                   k_fold = 5) %>% try()
  saveRDS(Perm_aft_Ncov,file = FileN_Ncov)
  rm(Perm_aft_Ncov)
}
...
if (FileN_Ncov %in% 
    list.files(paste(Fold_Name,Folder_args,sep = ""),full.names = T) == F){
  Perm_deephit_Ncov = permfit_survival(train = Train_Ncov,n_perm =100,method = "DeepHit",
                                       k_fold = 5,activation = "relu",
                                       frac = 0.2,early_stopping = T,
                                       num_nodes = c(50L, 40L, 30L,20L),epochs = 1000,
                                       batch_size = 50) %>% try()
  saveRDS(Perm_deephit_Ncov,file = FileN_Ncov)
  rm(Perm_deephit_Ncov)
}
...
if (FileN_Ncov %in% 
    list.files(paste(Fold_Name,Folder_args,sep = ""),full.names = T) == F){
  Perm_deepsurv_Ncov = permfit_survival(train = Train_Ncov,n_perm =100,method = "DeepSurv",
                                        k_fold = 5,activation = "relu",
                                        frac = 0.2,early_stopping = T,
                                        num_nodes = c(50L, 40L, 30L,20L),epochs = 1000,
                                        batch_size = 50) %>% try()
  saveRDS(Perm_deepsurv_Ncov,file = FileN_Ncov)
  rm(Perm_deepsurv_Ncov)
}
